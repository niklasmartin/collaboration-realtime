...
(function (root) {
  ...
  function buildV2SharedCanvas() {
    ...
    scene.appendActivity('Propose solutions requested', 2750);

    // Agent appears offset right of the problem (not on it).
    scene.showCursor('agent', 3100, { fade: 220, label: 'Research agent' });
    scene.moveCursor('agent', { x: 480, y: 5 }, { x: 456, y: 36 }, 3100, 650, { arc: 12 });
    scene.selectNode('problem', 3900, true, { color: '#5e6ad2' });
    scene.showIntent('agent', 'Reading', 4000, { anchor: { x: 456, y: 60 }, duration: 1400 });
    scene.showIntent('agent', 'Thinking', 5500, { anchor: { x: 456, y: 60 }, duration: 2800 });
    ...
    scene.typewriterLabel('userSolution', 'Onboarding emails', 7700, { step: 70 });
    ...
    scene.createNode('agentSolution', 10300, {
      ...
      label: 'Win-back offer', kind: 'provisional', duration: 360,
    });
    scene.drawEdge('e-problem-agent', 'problem', 'agentSolution', 10660, 500, {
      fromAnchor: 'bottom', toAnchor: 'top',
    });
    scene.appendActivity('Solution proposed', 10300);
    ...
    scene.appendActivity('Solution approved', 13800);
    ...
    scene.appendActivity('Add research requested', 16200);
    ...
    scene.showIntent('agent', 'Reading', 17500, { anchor: { x: 456, y: 60 }, duration: 1600 });
    ...
    scene.addNestedFinding(
      'userSolution',
      'Welcome series ↑ retention 22%',
      19900,
      { typewriter: true, step: 50, fade: 280 }
    );
    scene.appendActivity('Research proposed', 19900);
    ...
    scene.appendActivity('Research confirmed', 22400);
