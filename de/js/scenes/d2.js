...
(function (root) {
  ...
  function buildD2() {
    ...
    scene.moveCursor('lww-b', { x: 238, y: 215 }, { x: 90, y: 205 }, 3000, 650, { arc: 0 });
    scene.moveNode('lww-item', { x: 35, y: 184 }, { x: 110, y: 184 }, 3800, 700, { easing: 'easeInOut' });
    scene.moveCursor('lww-b', { x: 90, y: 205 }, { x: 150, y: 205 }, 3800, 700, { arc: 0 });
    scene.selectNode('lww-item', 5000, false);
    scene.hideCursor('lww-a', 5200, { fade: 180 });
    scene.hideCursor('lww-b', 5200, { fade: 180 });
    scene.appendActivity('✅ Späterer Wert: B ersetzt die Position von A', 4700);
    ...
    scene.setStampText('lock-state', '🔒 Niklas · 3 s', 6800, { kind: 'agent' });

    scene.showCursor('lock-b', 7200, { label: 'Sie', fade: 160 });
    scene.moveCursor('lock-b', { x: 500, y: 215 }, { x: 432, y: 205 }, 7200, 550, { arc: 0 });
    scene.setStampText('lock-state', '🔒 Niklas · 2 s', 8000, { kind: 'agent' });
    scene.setStampText('lock-state', '🔒 Niklas · 1 s', 9000, { kind: 'agent' });
    ...
    scene.setStampText('lock-state', '🔒 Sie · 3 s', 11400, { kind: 'human' });
    scene.appendActivity('🔓 Sperre aufgehoben — Sie übernehmen die Bearbeitung', 11400);

    // Panel 3: the object exposes two distinct properties and each actor
    // lands directly on the property they change.
    scene.node('field-card', { x: 580, y: 180, w: 172, h: 72, label: 'Titel: Onboarding-E-Mails', sublabel: 'Status: Entwurf', opacity: 1 });
    scene.cursor('field-a', { kind: 'human', x: 560, y: 112, label: '', opacity: 0 });
    scene.cursor('field-b', { kind: 'human', x: 774, y: 350, label: '', labelBelow: true, opacity: 0 });

    scene.showCursor('field-a', 12600, { label: 'Nutzer A', fade: 160 });
    scene.moveCursor('field-a', { x: 560, y: 112 }, { x: 666, y: 200 }, 12600, 650, { arc: 0 });
    scene.selectNode('field-card', 13300, true, { color: '#a0a0ab' });
    scene.setLabel('field-card', 'Titel: Willkommensserie', 13700);
    scene.selectNode('field-card', 14200, false);

    scene.showCursor('field-b', 14200, { label: 'Nutzer B', fade: 160 });
    scene.moveCursor('field-b', { x: 774, y: 350 }, { x: 666, y: 230 }, 14200, 650, { arc: 0 });
    scene.selectNode('field-card', 14900, true, { color: '#a0a0ab' });
    scene.setSublabel('field-card', 'Status: Geprüft', 15300);
    scene.selectNode('field-card', 15800, false);
    scene.appendActivity('✅ Getrennte Felder: Titel und Status bleiben erhalten', 16400);
