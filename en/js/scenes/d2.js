...
(function (root) {
  ...
  function buildD2() {
    ...
    scene.region('r1', { x: 15, y: 28, w: 238, h: 374, label: '1  LATER VALUE WINS', opacity: 1 });
    scene.region('r2', { x: 281, y: 28, w: 238, h: 374, label: '2  TEMPORARY LOCK', opacity: 1 });
    scene.region('r3', { x: 547, y: 28, w: 238, h: 374, label: '3  MERGE SEPARATE FIELDS', opacity: 1 });

    // Panel 1: two destinations, with the later move replacing the first.
    scene.node('lww-item', { x: 78, y: 184, w: 112, h: 52, label: 'Item', opacity: 1 });
    scene.node('lww-left', { x: 35, y: 304, w: 82, h: 32, label: 'A position', kind: 'ghost', opacity: 1 });
    scene.node('lww-right', { x: 151, y: 304, w: 82, h: 32, label: 'B position', kind: 'ghost', opacity: 1 });
    scene.cursor('lww-a', { kind: 'human', x: 30, y: 215, label: '', opacity: 0 });
    scene.cursor('lww-b', { kind: 'human', x: 238, y: 215, label: '', opacity: 0 });

    scene.showCursor('lww-a', 900, { label: 'User A', fade: 160 });
    ...
    scene.showIntent(null, 'A saved', 2500, { anchor: { x: 76, y: 372 }, duration: 1100 });

    scene.showCursor('lww-b', 3000, { label: 'User B', fade: 160 });
    ...
    scene.showIntent(null, 'B saved later -> final', 4700, { anchor: { x: 134, y: 372 }, duration: 1500 });
    scene.selectNode('lww-item', 5000, false);
    scene.hideCursor('lww-a', 5200, { fade: 180 });
    scene.hideCursor('lww-b', 5200, { fade: 180 });
    scene.appendActivity('Later value wins: B replaces A\'s position', 4700);
    ...
    scene.stamp('lock-state', { x: 400, y: 286, text: 'UNLOCKED', kind: 'human', opacity: 1 });
    ...
    scene.setStampText('lock-state', 'LOCKED by Niklas · 3s', 6800, { kind: 'agent' });

    scene.showCursor('lock-b', 7200, { label: 'You', fade: 160 });
    scene.moveCursor('lock-b', { x: 500, y: 215 }, { x: 432, y: 205 }, 7200, 550, { arc: 0 });
    scene.showIntent(null, 'blocked — wait', 7850, { anchor: { x: 448, y: 170 }, duration: 2100 });
    scene.setStampText('lock-state', 'LOCKED by Niklas · 2s', 8000, { kind: 'agent' });
    scene.setStampText('lock-state', 'LOCKED by Niklas · 1s', 9000, { kind: 'agent' });
    scene.setStampText('lock-state', 'UNLOCKED', 10000, { kind: 'human' });
    scene.selectNode('lock-item', 10000, false);
    scene.hideCursor('lock-a', 10000, { fade: 180 });
    scene.showIntent(null, 'lock released', 10050, { anchor: { x: 400, y: 330 }, duration: 900 });

    scene.moveCursor('lock-b', { x: 432, y: 205 }, { x: 392, y: 205 }, 10900, 450, { arc: 0 });
    scene.selectNode('lock-item', 11400, true, { color: '#a0a0ab' });
    scene.setStampText('lock-state', 'LOCKED by You · 3s', 11400, { kind: 'human' });
    scene.showIntent(null, 'your edit can begin', 11600, { anchor: { x: 400, y: 350 }, duration: 1500 });
    scene.appendActivity('Temporary lock: Niklas releases it, then You acquire it', 11400);

    // Panel 3: the object exposes two distinct properties and each actor
    // lands directly on the property they change.
    scene.node('field-title', { x: 580, y: 158, w: 172, h: 56, label: 'Title: Onboarding emails', opacity: 1 });
    scene.node('field-status', { x: 580, y: 246, w: 172, h: 56, label: 'Status: Draft', opacity: 1 });
    scene.cursor('field-a', { kind: 'human', x: 560, y: 112, label: '', opacity: 0 });
    scene.cursor('field-b', { kind: 'human', x: 774, y: 350, label: '', opacity: 0 });

    scene.showCursor('field-a', 12600, { label: 'User A', fade: 160 });
    scene.moveCursor('field-a', { x: 560, y: 112 }, { x: 666, y: 186 }, 12600, 650, { arc: 0 });
    scene.selectNode('field-title', 13300, true, { color: '#a0a0ab' });
    scene.setLabel('field-title', 'Title: Welcome series', 13700);
    scene.showIntent(null, 'title saved', 13900, { anchor: { x: 666, y: 236 }, duration: 1000 });
    scene.selectNode('field-title', 14200, false);

    scene.showCursor('field-b', 14200, { label: 'User B', fade: 160 });
    scene.moveCursor('field-b', { x: 774, y: 350 }, { x: 666, y: 274 }, 14200, 650, { arc: 0 });
    scene.selectNode('field-status', 14900, true, { color: '#a0a0ab' });
    scene.setLabel('field-status', 'Status: Reviewed', 15300);
    scene.showIntent(null, 'status saved', 15500, { anchor: { x: 666, y: 330 }, duration: 900 });
    scene.selectNode('field-status', 15800, false);
    scene.showIntent(null, 'BOTH FIELDS KEPT', 16400, { anchor: { x: 666, y: 378 }, duration: 1500 });
    scene.appendActivity('Separate fields: A\'s title and B\'s status both survive', 16400);
