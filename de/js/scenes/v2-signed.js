...
(function (root) {
  ...
  function buildV2Signed() {
    ...
    scene.button('proposeSolution', { x: TRIG1.x, y: TRIG1.y, w: TRIG1.w, h: TRIG1.h, label: 'Lösung vorschlagen' });
    scene.button('addResearch', { x: TRIG2.x, y: TRIG2.y, w: TRIG2.w, h: TRIG2.h, label: 'Recherche hinzufügen', opacity: 0 });
    ...
    scene.stamp('problemStamp', { x: 400, y: 72, text: 'Sie', kind: 'human' });
    scene.stamp('solutionStamp', { x: 400, y: 216, text: 'Sie', kind: 'human' });
    scene.stamp('evidenceStamp', { x: 400, y: 352, text: 'vorgeschlagen · Recherche-Agent', kind: 'agent' });
    ...
    scene.createNode('problem', 300, {
      ...
      label: 'Warum wandern Nutzer ab?',
      duration: 300,
    });
    scene.showStamp('problemStamp', 500);

    scene.showCursor('user', 1000, { fade: 150, label: 'Sie' });
    ...
    scene.appendActivity('Recherche hinzufügen', 5600);

    // ======================================================================
    // Agent enters
    // ======================================================================

    scene.showCursor('agent', 5900, { fade: 200, label: 'Recherche-Agent' });
    scene.moveCursor('agent', { x: 750, y: 40 }, { x: 340, y: 44 }, 5900, 800, { arc: 20 });
    scene.selectNode('problem', 5900, true, { color: '#5e6ad2' });
    scene.showIntent('agent', 'Liest', 6700, { anchor: { x: 340, y: 70 }, duration: 900 });
    ...
    scene.showIntent('agent', 'Fügt Nachweise hinzu', 8400, { anchor: { x: 340, y: 214 }, duration: 1000 });
    ...
    scene.createNode('evidence', 10100, {
      ...
      label: 'Willkommensserie ↑ Bindung 22 %',
      kind: 'provisional',
      duration: 300,
    });
    scene.showStamp('evidenceStamp', 10100, { text: 'vorgeschlagen · Recherche-Agent', kind: 'agent' });
    scene.appendActivity('Nachweis vorgeschlagen', 10100);

    scene.moveCursor('user', { x: 400, y: 252 }, { x: 430, y: 320 }, 10400, 800, { arc: -16 });
    scene.showIntent('agent', 'Pausiert', 11600, { anchor: { x: 430, y: 288 }, duration: 900 });
    ...
    scene.setStampText('evidenceStamp', 'freigegeben · Sie', 13100, { kind: 'human' });
    scene.appendActivity('Nachweis freigegeben', 13100);
