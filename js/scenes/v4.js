...
(function (root) {
  ...
  function buildV4() {
    ...
    scene.region('state-only', { x: 18, y: 34, w: 352, h: 372, label: 'WITHOUT SIGNALS', opacity: 1 });
    scene.region('with-context', { x: 430, y: 34, w: 352, h: 372, label: 'WITH SIGNALS', opacity: 1 });
    scene.node('left-heading', { x: 42, y: 66, w: 304, h: 34, label: 'State changes, cause is hidden', opacity: 1 });
    scene.node('right-heading', { x: 454, y: 66, w: 304, h: 34, label: 'State changes, cause is visible', opacity: 1 });
    ...
    ['left', 'right'].forEach(function (side) {
      var p = layouts[side];
      scene.node(side + '-problem', { x: p.problem.x, y: p.problem.y, w: p.problem.w, h: p.problem.h, label: 'Why are customers churning?', opacity: 1 });
      scene.node(side + '-user', { x: p.user.x, y: p.user.y, w: p.user.w, h: p.user.h, label: 'Onboarding emails', opacity: 1 });
      scene.node(side + '-agent', { x: p.agent.x, y: p.agent.y, w: p.agent.w, h: p.agent.h, label: 'Win-back offer', opacity: 1 });
    ...
    scene.stamp('evidence-stamp', { x: 689, y: 357, text: 'Research · proposed', kind: 'agent', opacity: 0 });
    scene.node('history', { x: 454, y: 368, w: 304, h: 26, label: 'History will explain completed changes', opacity: 1 });

    // 1. The same card moves on both sides. Only the right explains it.
    scene.showCursor('you', 1200, { label: 'You', fade: 180 });
    ...
    scene.setLabel('history', 'History: You moved Onboarding emails', 3300);
    ...
    scene.showCursor('research', 4100, { label: 'Research agent', fade: 180 });
    scene.moveCursor('research', { x: 746, y: 350 }, { x: 688, y: 314 }, 4100, 600, { arc: 0 });
    scene.createNode('left-evidence', 4900, { x: 206, y: 292, w: 142, h: 44, label: 'Evidence: 2 studies', duration: 350 });
    scene.createNode('right-evidence', 4900, { x: 618, y: 292, w: 142, h: 44, label: 'Evidence: 2 studies', kind: 'provisional', duration: 350 });
    scene.showStamp('evidence-stamp', 5000, { text: 'Research · proposed', kind: 'agent', fade: 180 });
    scene.setLabel('history', 'History: Research agent added evidence', 5700);
    scene.hideCursor('research', 6000, { fade: 180 });

    // 3. The same connection appears. Selection and actor explain it right.
    scene.showCursor('structure', 6500, { label: 'Structure agent', fade: 180 });
    ...
    scene.setLabel('history', 'History: Structure agent connected evidence', 8500);
    ...
    scene.setLabel('history', 'Presence: Maya is reading the question', 10300);
    scene.showIntent(null, 'same document state', 11100, { anchor: { x: 194, y: 390 }, duration: 1300 });
    scene.showIntent(null, 'understandable activity', 11100, { anchor: { x: 606, y: 426 }, duration: 1300 });

    scene.appendActivity('Left: silent changes · Right: actor, intent, provisional state, and history', 11200);
    return scene.build();
  }
