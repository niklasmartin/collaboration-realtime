...
(function (root) {
  ...
  function buildD4() {
    ...
    scene.node('source', { x: 310, y: 22, w: 180, h: 36, label: 'Remote node changes', opacity: 1 });
    scene.region('manual', { x: left.x, y: left.y, w: left.w, h: left.h, label: 'YOUR APPLICATION', opacity: 1 });
    scene.region('subscribed', { x: right.x, y: right.y, w: right.w, h: right.h, label: 'SYNC LAYER', opacity: 1 });
    scene.node('manual-title', { x: 46, y: 102, w: 318, h: 34, label: 'Handle every incoming event', opacity: 1 });
    scene.node('sync-title', { x: 436, y: 102, w: 318, h: 34, label: 'Subscribe once to nodes + edges', opacity: 1 });

    var steps = [
      { id: 'receive', y: 150, label: '1  Receive event' },
      { id: 'inspect', y: 202, label: '2  Read payload' },
      { id: 'patch', y: 254, label: '3  Patch local cache' },
      { id: 'reconcile', y: 306, label: '4  Reconcile optimistic state' },
    ];
    ...
    scene.node('manual-result', { x: 88, y: 358, w: 234, h: 38, label: 'UI finally becomes current', opacity: 1 });

    scene.node('query', { x: 468, y: 174, w: 254, h: 54, label: 'Live query', sublabel: 'nodes {}, edges {}', opacity: 1 });
    scene.node('view', { x: 468, y: 300, w: 254, h: 54, label: 'UI reads current result', opacity: 1 });
    ...
    scene.createNode('left-event', 1000, { x: 182, y: 36, w: 78, h: 28, label: 'change', kind: 'provisional', duration: 160 });
    scene.moveNode('left-event', { x: 182, y: 36 }, { x: 166, y: 154 }, 1160, 850, { easing: 'easeInOut' });
    scene.removeNode('left-event', 2050, { duration: 140 });

    scene.createNode('right-event', 1000, { x: 540, y: 36, w: 78, h: 28, label: 'change', kind: 'provisional', duration: 160 });
    ...
    scene.showIntent(null, 'current after app code runs', 6500, { anchor: { x: 205, y: 430 }, duration: 1800 });
    ...
    scene.createNode('fresh-result', 2800, { x: 554, y: 248, w: 82, h: 30, label: 'new result', kind: 'provisional', duration: 180 });
    ...
    scene.showIntent(null, 'current automatically', 4100, { anchor: { x: 595, y: 390 }, duration: 2300 });

    scene.appendActivity('Same remote change · left: application reconciles it · right: subscription stays current', 7200);
    return scene.build();
  }
